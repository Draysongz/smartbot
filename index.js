...
bot.start(async (ctx) => {
  ...
  if (referralData !== undefined) {
    ...
    const existingUser = await UserDetails.findOne({
      userId,
      referredUsers: ctx.from.id.toString(),
    })
    if (existingUser) {
    ...
    await UserDetails.findOneAndUpdate(
      ...
      {
        $inc: { referralCount: 1 },
        $addToSet: { referredUsers: ctx.from.id.toString() },
      },
...
ReferralScene.enter(async (ctx) => {
  ...
  if (existingUser && existingUser.referralLink !== "null") {
    const truncatedWalletAddress = `${existingUser.walletAddress.slice(
      0,
      4
    )}...${existingUser.walletAddress.slice(-6)}`
    ...
    await ctx.reply(referralMessage, {
      reply_markup: {
        inline_keyboard: [
          ...
          [
            {
              text: `Referral Link Wallet: ${truncatedWalletAddress}`,
    ...
    await ctx.reply(
      "It seems you don't have a referral link yet. Please generate one to start referring your friends!",
      {
        reply_markup: {
          inline_keyboard: [
